...
fn dispatch_normal(
    ...
    if alternatives > 0 {
        ...
        return Ok(format_multi_response(answers, format, colors));
    }
...
/// Color palette for multi-route visualization (simplestyle-spec stroke values).
const ROUTE_COLORS: &[&str] = &[
    "#ff5500", "#0055ff", "#00aa44", "#aa00cc", "#cc8800", "#e6194b", "#3cb44b", "#4363d8", "#f58231", "#911eb4",
];

/// Format multiple route answers into a response.
fn format_multi_response(answers: Vec<QueryAnswer>, format: Option<&str>, colors: bool) -> Value {
    match format {
        Some("json") => {
        ...
        _ => answers_to_geojson(answers, colors),
    }
}

/// Build a GeoJSON FeatureCollection with one LineString Feature per route.
fn answers_to_geojson(answers: Vec<QueryAnswer>, colors: bool) -> Value {
    if answers.is_empty() {
    ...
    let features: Vec<Value> = answers
        ...
        .map(|(idx, a)| {
            ...
            let obj = props.as_object_mut().unwrap();
            if let Some((lat, lng)) = a.origin {
                obj.insert("origin".into(), serde_json::json!([lat, lng]));
            }
            if let Some((lat, lng)) = a.destination {
                obj.insert("destination".into(), serde_json::json!([lat, lng]));
            }
            if !a.turns.is_empty() {
                obj.insert("turns".into(), serde_json::to_value(a.turns).unwrap());
            }
            if colors {
