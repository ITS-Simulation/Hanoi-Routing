...
bool is_osm_way_used_by_motorcycles(uint64_t osm_way_id, const TagMap&tags, std::function<void(const std::string&)>log_message){
	const char*motorcycle = tags["motorcycle"];
	if(motorcycle && str_eq(motorcycle, "no"))
		return false;
	...
	if(access){
		if(!(str_eq(access, "yes") || str_eq(access, "permissive") || str_eq(access, "delivery") || str_eq(access, "designated") || str_eq(access, "destination"))){
			// OSM's mode-specific tags can override a generic access tag.
			bool explicit_motorcycle_override = (motorcycle && str_eq(motorcycle, "yes")) || (motor_vehicle && str_eq(motor_vehicle, "yes"));
			if(!explicit_motorcycle_override)
				return false;
		}
	}
	...
	if(str_eq(highway, "bicycle_road")){
		if(motorcycle != nullptr && str_eq(motorcycle, "yes"))
			return true;
		return false;
	}
	...
	const char* maxspeed_motorcycle = tags["maxspeed:motorcycle"];
	if(maxspeed_motorcycle != nullptr)
		return true;
...
OSMWayDirectionCategory get_osm_motorcycle_direction_category(uint64_t osm_way_id, const TagMap&tags, std::function<void(const std::string&)>log_message){
	const char*oneway_motorcycle = tags["oneway:motorcycle"];
	if(oneway_motorcycle != nullptr){
		if(str_eq(oneway_motorcycle, "-1") || str_eq(oneway_motorcycle, "opposite"))
			return OSMWayDirectionCategory::only_open_backwards;

		if(str_eq(oneway_motorcycle, "1") || str_eq(oneway_motorcycle, "yes") || str_eq(oneway_motorcycle, "true"))
			return OSMWayDirectionCategory::only_open_forwards;

		if(str_eq(oneway_motorcycle, "0") || str_eq(oneway_motorcycle, "no") || str_eq(oneway_motorcycle, "false"))
			return OSMWayDirectionCategory::open_in_both;

		if(log_message)
			log_message("Warning: OSM way "+std::to_string(osm_way_id)+" has unknown oneway tag value \""+oneway_motorcycle+"\" for \"oneway:motorcycle\". Way is closed.");
		return OSMWayDirectionCategory::closed;
	}

	return get_osm_car_direction_category(osm_way_id, tags, log_message);
}
...
namespace{
	...
	unsigned parse_osm_speed_tag(uint64_t osm_way_id, const char*speed_tag_value, std::function<void(const std::string&)>log_message){
		if(speed_tag_value == nullptr || str_eq(speed_tag_value, "unposted"))
			return inf_weight;

		char lower_case_maxspeed[1024];
		copy_str_and_make_lower_case(speed_tag_value, lower_case_maxspeed, sizeof(lower_case_maxspeed)-1);
	...
	bool except_tag_contains_motorcycle_or_motor_vehicle(const char*except){
		if(except == nullptr)
			return false;

		char lower_case_except[1024];
		copy_str_and_make_lower_case(except, lower_case_except, sizeof(lower_case_except)-1);

		bool has_motorcycle_exception = false;
		split_str_at_osm_value_separators(
			lower_case_except,
			[&](const char*except_entry){
				if(str_eq(except_entry, "motorcycle") || str_eq(except_entry, "motor_vehicle"))
					has_motorcycle_exception = true;
			}
		);

		return has_motorcycle_exception;
	}
}
...
unsigned get_osm_motorcycle_way_speed(uint64_t osm_way_id, const TagMap&tags, std::function<void(const std::string&)>log_message){
	unsigned speed = parse_osm_speed_tag(osm_way_id, tags["maxspeed:motorcycle"], log_message);
	if(speed != inf_weight)
		return speed;

	speed = parse_osm_speed_tag(osm_way_id, tags["maxspeed"], log_message);
	if(speed != inf_weight)
		return speed;
	...
	auto maxspeed_motorcycle = tags["maxspeed:motorcycle"];
	auto maxspeed = tags["maxspeed"];
	if(maxspeed_motorcycle && highway && log_message)
		log_message("Warning: OSM way "+std::to_string(osm_way_id) +" has an unrecognized \"maxspeed:motorcycle\" tag of \""+maxspeed_motorcycle+"\" and an unrecognized \"highway\" tag of \""+highway+"\" and an no junction tag -> assuming 50km/h.");
	if(maxspeed && !maxspeed_motorcycle && highway && log_message)
		log_message("Warning: OSM way "+std::to_string(osm_way_id) +" has an unrecognized \"maxspeed\" tag of \""+maxspeed+"\" and an unrecognized \"highway\" tag of \""+highway+"\" and an no junction tag -> assuming 50km/h.");
	if(!maxspeed_motorcycle && !maxspeed && highway && log_message)
		log_message("Warning: OSM way "+std::to_string(osm_way_id) +" has no \"maxspeed:motorcycle\" or \"maxspeed\" and an unrecognized \"highway\" tag of \""+highway+"\" and an no junction tag -> assuming 50km/h.");
	if(maxspeed_motorcycle && !highway && log_message)
		log_message("Warning: OSM way "+std::to_string(osm_way_id) +" has an unrecognized \"maxspeed:motorcycle\" tag of \""+maxspeed_motorcycle+"\" and no \"highway\" tag and an no junction tag -> assuming 50km/h.");
	if(!maxspeed_motorcycle && maxspeed && !highway && log_message)
		log_message("Warning: OSM way "+std::to_string(osm_way_id) +" has an unrecognized \"maxspeed\" tag of \""+maxspeed+"\" and no \"highway\" tag and an no junction tag -> assuming 50km/h.");
	if(!maxspeed_motorcycle && !maxspeed && !highway && log_message)
		log_message("Warning: OSM way "+std::to_string(osm_way_id) +" has no \"maxspeed:motorcycle\", no \"maxspeed\", and no \"highway\" tag and an no junction tag -> assuming 50km/h.");
...
void decode_osm_motorcycle_turn_restrictions(
	uint64_t osm_relation_id, const std::vector<OSMRelationMember>&member_list,
	const TagMap&tags,
	std::function<void(OSMTurnRestriction)>on_new_turn_restriction,
	std::function<void(const std::string&)>log_message
){
	const char*restriction = tags["restriction:motorcycle"];
	bool is_general_restriction = false;
	if(restriction == nullptr){
		restriction = tags["restriction"];
		is_general_restriction = true;
	}

	if(restriction == nullptr)
		return;

	if(is_general_restriction && except_tag_contains_motorcycle_or_motor_vehicle(tags["except"]))
		return;
