...
3. [CLI Reference (cch-hanoi)](#3-cli-reference-cch-hanoi)
4. [Tùy chỉnh trọng số (Customization)](#4-tùy-chỉnh-trọng-số-customization)
5. [Benchmark & Phân tích hiệu năng](#5-benchmark--phân-tích-hiệu-năng)
6. [Gateway đa profile](#6-gateway-đa-profile)
7. [Định dạng Output](#7-định-dạng-output)
8. [Luồng nghiệp vụ thường gặp](#8-luồng-nghiệp-vụ-thường-gặp)
9. [Hiệu năng tham khảo](#9-hiệu-năng-tham-khảo)
...
| ----------------- | ------- | ------------------------------------------- |
| `hanoi_server`    | Binary  | HTTP server REST API (query + customize)    |
...
### 2.1 Endpoints tổng quan

Base URL mặc định: `http://localhost:8081` (query port)

| Endpoint     | Method | Port  | Mô tả                          |
| ------------ | ------ | ----- | ------------------------------- |
| `/query`     | POST   | 8081  | Tìm đường ngắn nhất            |
| `/info`      | GET    | 8081  | Metadata đồ thị                |
| `/health`    | GET    | 8081  | Trạng thái hoạt động            |
| `/ready`     | GET    | 8081  | Kiểm tra sẵn sàng phục vụ      |
| `/customize` | POST   | 9081  | Upload trọng số tùy chỉnh      |

### 2.2 POST /query — Tìm đường
...
### 2.4 GET /health — Trạng thái hoạt động
...
### 2.5 GET /ready — Sẵn sàng phục vụ
...
### 2.6 POST /customize — Upload trọng số (port 9081)

Upload vector trọng số mới (little-endian `u32`, mỗi edge 4 bytes):

```bash
# Upload file raw binary
curl -X POST http://localhost:9081/customize \
  --data-binary @custom_weights.bin

# Upload với nén gzip
gzip -c custom_weights.bin | curl -X POST http://localhost:9081/customize \
...
## 4. Tùy chỉnh trọng số (Customization)
...
# Lấy num_edges
info = requests.get("http://localhost:8081/info").json()
...
resp = requests.post("http://localhost:9081/customize", data=data)
...
## 5. Benchmark & Phân tích hiệu năng

### 5.1 bench_core — Benchmark thuật toán
...
### 5.2 bench_server — Benchmark HTTP
...
bench_server --url http://localhost:8081 --queries 500
```
...
### 5.3 bench_report — So sánh kết quả
...
## 8. Luồng nghiệp vụ thường gặp

### 8.1 Cập nhật bản đồ mới
...
### 8.3 Cập nhật trọng số real-time
...
# 2. Upload trọng số
curl -X POST http://localhost:9081/customize --data-binary @live_weights.bin
...
### 8.4 Regression test sau khi thay đổi code
