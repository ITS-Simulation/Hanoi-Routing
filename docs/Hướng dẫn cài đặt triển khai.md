...
| `--camera-config`      | `CCH_Data_Pipeline/config/mvp_camera.yaml` | File cấu hình camera YAML |
| `--query-port`         | `8080`   | Cổng REST API cho query                         |
| `--customize-port`     | `9080`   | Cổng cho upload trọng số tùy chỉnh             |
| `--serve-ui`           | `false`  | Serve route-viewer UI tại `/` và `/ui`          |
